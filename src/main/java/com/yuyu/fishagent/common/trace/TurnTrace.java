 ...
 * 单轮对话执行 trace。
 *
 * <p>只存节点片段、prompt 指纹与统计指标，不保存完整 prompt，兼顾排障价值和存储边界。
...
public class TurnTrace {
    ...
    /**
     * 发送给 AI 的关键 prompt 指纹与统计。
     *
     * <p>与节点共用同一套顺序号，便于按时间轴还原“在哪个阶段发起了模型调用，随后得到了什么节点输出”。</p>
     */
    private List<PromptCall> prompts = new CopyOnWriteArrayList<>();
    ...
    @Data
    public static class PromptCall {
        private int order;
        ...
        private int toolChars;
        private int otherChars;
        private Integer ragInjectionCount;
